...
export const authRoutes = async (fastify: FastifyInstance) => {
  ...
  fastify.get('/auth/login/google/callback', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if ('error' in user && token.token.id_token) {
        ...
        if (decoded.email && decoded.name) {
          ...
          return reply.redirect('/auth/login/failure');
        }
      }
      ...
      reply.redirect('/auth/login/failure');
    }
  });

  // fastify.get('/auth/login/success', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  // fastify.get('/auth/login/failure', async (request: FastifyRequest, reply: FastifyReply) => {
  //   reply.type('text/html').send('<h1>Google OAuth2 login failed.</h1>');
  // });

  // Optional: Protected route for SPA to check auth status
  fastify.get('/auth/api/me', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
