...
export const createServer = (config: AppConfig): FastifyInstance => {
  ...
  // Hook global per difusió SSE d'accions d'usuari
  server.addHook('onSend', async (request: any, reply: any, payload: any) => {
    const url = request.url;
    if (
      (request.method === 'POST' && url.startsWith('/api/users')) ||
      (request.method === 'PUT' && url.startsWith('/api/users/')) ||
      (request.method === 'POST' && url.startsWith('/api/auth/login'))
    ) {
      setTimeout(async () => {
        try {
          const { broadcastGlobalSseEvent } = await import('./routes/globalSse.js');
          broadcastGlobalSseEvent('users:updated', { reason: 'user create/update/login', url, method: request.method });
        } catch (e) {
          server.log.error('Failed to broadcast SSE event:', undefined, String(e));
        }
      }, 0);
    }
  });
  ...
  server.addHook('preHandler', async (request: any, reply: any) => {
    ...
    if (
      ...
      !request.url.startsWith('/api/users') && // users endpoint is public
      request.url !== '/api/sse/global' // SSE global endpoint is public
    ) {
